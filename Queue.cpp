...
void Queue::push(Process * & A)
{
	q[bot] = A;				//Put new process into bottom of queue
	qsize++;				//Increment size count
	if(bot < QUEUE_SIZE - 1){		//Check if near full. If not, increment size
		bot++;
	}
...
Process * Queue::pop()
{
	Process * A;				//Declare Process variable
	A = q[top];				//Set it to top of queue
	q[top] = 0;				//Set location to 0
	qsize--;				//Decrement size count
	if(top < QUEUE_SIZE - 1){		//Check if top is 1 below max size
...
void Queue::PrintQ()
{
	if(qsize != 0){					//Check for non empty queue
		int i;
		for(i = 0; i < QUEUE_SIZE; i++){	//go through queue
